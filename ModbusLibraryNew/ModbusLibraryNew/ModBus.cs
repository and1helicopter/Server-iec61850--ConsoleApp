using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ModbusLibraryNew
{
    public static class ModBus
    {
        #region CRC_TABLE
        static byte[] tableCRCLo = {
        
        };

        static byte[] tableCRCHi = {
        
        };
        #endregion

        public static void CalcCRC(byte[] buffer, int count, out byte crcLo, out byte crcHi)
        {
            short i = 0;
            short crcIndex = 0;
            byte crcHi1 = 0xFF;
            byte crcLo1 = 0xFF;


            for (i = 0; i < count; i++)
            {
                crcIndex = (short)(crcLo1 ^ (buffer[i]));
                crcLo1 = (byte)(crcHi1 ^ tableCRCLo[crcIndex]);
                crcHi1 = tableCRCHi[crcIndex];
            }

            crcLo = crcLo1;
            crcHi = crcHi1;
        }
        public static void CalcTxBuffer(byte[] buffer, int count, out byte[] txBuffer)
        {
            byte crcLo, crcHi;
            CalcCRC(buffer, count, out crcLo, out crcHi);
            txBuffer = new byte[count + 2];
            for (int i = 0; i < count; i++)
            {
                txBuffer[i] = buffer[i];
            }
            txBuffer[count] = crcLo;
            txBuffer[count + 1] = crcHi;
        }
        public static bool CheckCRC(byte[] buffer, int count)
        {
            if (buffer.Length < count) { return false; }

            byte crcLo, crcHi;
            CalcCRC(buffer, count, out crcLo, out crcHi);
            if (crcLo != 0) return false;
            if (crcHi != 0) return false;
            return true;
        }
        private static bool CheckCRCAndRemoveData(byte[] buffer, int byteCount, int dataOffset, int dataCount, out ushort[] outData)
        {
            if (!CheckCRC(buffer, byteCount))
            {
                outData = new ushort[0];
                return false;
            }

            if ((byteCount - 2) < (dataOffset + dataCount * 2))
            {
                outData = new ushort[0];
                return false;
            }

            int i;
            outData = new ushort[dataCount];
            for (i = 0; i < dataCount; i++)
            {
                outData[i] = (ushort)((((ushort)(buffer[dataOffset + i * 2])) << 8) | (ushort)(buffer[dataOffset + 1 + i * 2]));
            }
            return true;
        }
        public static void RemoveData(byte[] buffer, int dataOffset, int dataCount, out ushort[] outData)
        {
            if ((dataOffset + dataCount * 2) > buffer.Length) { outData = new ushort[0]; }
            int i;
            outData = new ushort[dataCount];
            try
            {
                for (i = 0; i < dataCount; i++)
                {
                    outData[i] = (ushort)((((ushort)(buffer[dataOffset + i * 2])) << 8) | (ushort)(buffer[dataOffset + 1 + i * 2]));
                }
            }
            catch
            {
                outData = new ushort[dataCount];
            }
        }


    }
}
